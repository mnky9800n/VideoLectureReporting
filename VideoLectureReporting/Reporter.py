...
) as pivottable""".format(get_pivot_date_range(Spring_data))
...
# Count by video type
#####
lab_videos = Spring_data[Spring_data['Lab Number'] > -1].groupby('Title').size()
lecture_videos = Spring_data[Spring_data['Lab Number'] == -1].groupby('Title').size()
...
Spring_data.groupby(['session_user_id','Video Order']).size()


# In[200]:

def get_pivot_date_range(dataframe):
    """
    We need a full date range for the pivot 
    table in SQL to create the date columns.
    
    NOTE: the date range is truncated based
    on the semester days.
    """
    ...
    return daterange[:119*13] +')'

get_pivot_date_range(Spring_data)
